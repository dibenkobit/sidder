...
beforeAll(async () => {
  dir = await mkdtemp(join(tmpdir(), 'sowme-load-'));
...
describe('isParseFailure', () => {
  ...
  test("a raised --stack-trace-limit does not turn Node's parse failure into a throw", () => {
    ...
    // and into sowme's own importing frame, so "did any user code appear in the stack"
    // is not the question. The location above the frames is.
    const error = nodeParseFailure();
    error.stack += ['', '    at file:///node_modules/sowme/dist/load-module.js:18:12'].join('\n');
