...
describe('formatStatus', () => {
  ...
  test('spells out what a seed with no transaction gave up besides atomicity', () => {
    ...
    // seed sidder cannot keep a concurrent run out of is the one that says `no transaction`.
...
describe('formatCrossImports', () => {
  ...
  test('does not claim to know which bindings are data and which are work', () => {
    ...
    expect(block).toContain('sidder does not decide');
...
describe('formatNothingApplied', () => {
  ...
  test('offers --force and forget when everything --only named is in the journal', () => {
    ...
    expect(block).toContain('sidder run --only demo --force');
    expect(block).toContain('sidder forget demo');
  });
  ...
  test('says so when nothing --only named is a seed at all', () => {
    ...
    expect(block).toContain('sidder status');
  });
  ...
  test('says nobody chose one when the environment is only the default', () => {
    ...
    expect(block).toContain('nothing chose an environment, so sidder used development');
