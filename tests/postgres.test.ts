...
const url = process.env['SIDDER_TEST_DATABASE_URL'];
const describeIf = url ? describe : describe.skip;

if (!url) {
  console.log('postgres.test.ts skipped — set SIDDER_TEST_DATABASE_URL to run it');
}

describeIf('against a real Postgres', () => {
  const pool = new Pool({ connectionString: url });
  const JOURNAL = 'sidder_test_journal';
  ...
  describe('pgAdapter', () => {
    ...
    /**
     * Two `sidder run` at once — two replicas in a deploy, two jobs in one pipeline.
     *
  ...
  /**
   * `journalTable` pointed at a table sidder did not create.
   *
  ...
  describe('a journal table that is not a journal', () => {
    ...
    test('a run against an application table says so, and says which setting to change', async () => {
      ...
      expect(message).toBe(`Table "${JOURNAL}" exists but is not sidder's journal`);
    ...
    test('a schema-qualified name is diagnosed too', async () => {
      // `assertSafeTableName` allows `public.sidder_journal`, so the introspection has to
    ...
    test('describes the table the read reached, which search_path may have chosen', async () => {
      ...
      // Here the name resolves to the wrong-shaped table in `sidder_alt` while a perfectly
      ...
      try {
        await client.query('create schema sidder_alt');
        await client.query(`create table sidder_alt.${JOURNAL} (id serial primary key)`);
        await ensureJournal(adapter.root, `public.${JOURNAL}`);
        await client.query('set search_path to sidder_alt, public');
        ...
        await client.query('drop schema sidder_alt cascade');
        client.release();
      }
    });
  });
