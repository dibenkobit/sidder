/**
 * Every type sidder has. If a concept is not in this file, sidder does not have it.
 */
...
export interface Adapter<TDb = unknown> {
   ...
   *
   * Leave the isolation level at the default. sidder decides whether a seed still needs
...
export interface Seed<TDb = unknown> {
   ...
   * and calling it. sidder guarantees each seed runs exactly once per invocation,
   ...
   * nowhere to hold one: sidder re-reads the journal immediately before running it, which
...
export interface Config<TDb = unknown> {
  ...
  /** Journal table name. Default: `'sidder_journal'`. */
  journalTable?: string;
}
 ...
 *
 * `dependsOn` exists to replace exactly this. sidder guarantees each seed runs once per
 ...
 * sidder ran it, once because demo called it. The second application is silent, because
 * both are ordinary writes.
 *
 * So sidder reports the coupling and names what it saw. It deliberately does not decide
 ...
 * what sidder does nor what it exits with. See `findCrossImports` in `cross-imports.ts`
...
export type RunEvent =
   ...
   *
   * Emitted once the database has refused sidder the seed's lock, never merely because one
   ...
   * no lock and so never reports this, which is a gap `sidder status` states rather than
   * papers over.
   */
